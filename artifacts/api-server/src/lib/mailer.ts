...
export async function sendPasswordResetOtpEmail(toEmail: string, otp: string, userName: string) {
  try {
    ...
    const info = await transport.sendMail({
      from: `"E-Ration Token System" <noreply@eration.karnataka.gov.in>`,
      to: toEmail,
      subject: "Password Reset OTP - E-Ration System",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f9f9f9; padding: 20px; border-radius: 8px;">
          <div style="background: #e65c00; padding: 20px; border-radius: 8px 8px 0 0; text-align: center;">
            <h2 style="color: white; margin: 0;">Government of Karnataka</h2>
    ...
    const previewUrl = nodemailer.getTestMessageUrl(info);
    if (previewUrl) {
      logger.info({ previewUrl }, "Password reset OTP email preview (Ethereal test account)");
    }
    logger.info({ to: toEmail, messageId: info.messageId }, "Password reset OTP email sent");
    return { success: true, previewUrl: previewUrl || null };
  } catch (err) {
